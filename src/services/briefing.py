    ...
    async def generate_morning_briefing(self, user: User) -> dict:
        ...
        if spark_email.is_configured:
            ...
            try:
                ...
                # Use twin (or generic AI fallback) to score urgency
                if all_urgent_emails:
                    all_urgent_emails = await twin_client.analyze_email_urgency(all_urgent_emails)
        ...
        # Use twin (or generic AI fallback) to generate priorities
        try:
            priorities = await twin_client.generate_daily_priorities(
